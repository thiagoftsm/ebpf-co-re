...
static __always_inline void netdata_shm_fill_event(struct netdata_shm_event_t NETDATA_ARENA_PTR *ev, void *ctrl)
{
    __u32 tgid = 0;
...
int netdata_syscall_shmget_buffer(struct pt_regs *ctx)
{
    ...
    struct netdata_shm_event_t NETDATA_ARENA_PTR *ev = bpf_ringbuf_reserve(&shm_events, sizeof(*ev), 0);
    if (!ev)
        return 0;
