...
static __always_inline void netdata_swap_fill_event(struct netdata_swap_event_t NETDATA_ARENA_PTR *ev, void *ctrl)
{
    __u32 tgid = 0;
...
int netdata_swap_read_folio_buffer(struct pt_regs *ctx)
{
    ...
    struct netdata_swap_event_t NETDATA_ARENA_PTR *ev = bpf_ringbuf_reserve(&swap_events, sizeof(*ev), 0);
    if (!ev)
        return 0;
