...
static __always_inline void netdata_dc_fill_event(struct netdata_dc_event_t NETDATA_ARENA_PTR *ev, void *ctrl)
{
    __u32 tgid = 0;
...
int netdata_lookup_fast_buffer(struct pt_regs *ctx)
{
    ...
    struct netdata_dc_event_t NETDATA_ARENA_PTR *ev = bpf_ringbuf_reserve(&dc_events, sizeof(*ev), 0);
    if (!ev)
        return 0;
